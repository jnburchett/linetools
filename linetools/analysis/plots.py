...
def stack_plot(abslines, vlim=[-300,300.]*u.km/u.s, nrow=6, show=True, spec=None,
               ...
               tight_layout=False, add_ew=False, zref=None, labelloc=(0.1,0.1)):
    ...
    labelloc : tuple, optional
      Location of transition label in axes coordinates (x,y)
    ...
    for qq, iline in enumerate(gdiline):
        ...
        # Label
        ax.text(labelloc[0], labelloc[1], iline.data['name'], transform=ax.transAxes, ha='left', va='center')#, fontsize='large')  # , bbox={'facecolor':'white'})
        if add_ew:
